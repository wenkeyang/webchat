...
socket.on("user_list",function(userList){    // 获取用户列表并展示
	...
	for(var i=0;i<userList.length;i++){ 
		...
		$(".user-list").append("<tr class='row'><td class='col-sm-1'><img style='width:10px; height:20px;' src="+imgSrc+"></td><td class='col-sm-11 user-name' title='Start Private Chat!~' onclick='toUser(this)'>"+userList[i].name+"</td></tr>");
	}
	var listCount = $(".user-list").find("tr").length;
	$("#list-count").text("Current Online：" + listCount + " Person");
});
...
socket.on("user_say",function(name,time,content){    // 获取用户的聊天信息并显示于面板
	...
	var msg_list = $(".msg-list");
	msg_list.append( 
		'<div class="msg-wrap"><div class="msg-info"><span class="msg-name" title="Private Chat~" onclick="toUser(this)">'+name+' </span>'+
...
socket.on("sayToYou",function(fromuser,content){ 
	var msg_list = $(".msg-list");
		msg_list.append( 
		'<div class="msg-wrap"><div class="msg-content msg-system">'+
		'【'+fromuser+'】send u private msg：'+content+'</div></div>'
	);
	notifyMe();
});
socket.on("say_private_done",function(touser,content){ 
	var msg_list = $(".msg-list");
		msg_list.append( 
		'<div class="msg-wrap"><div class="msg-content msg-system">'+
		'You send 【'+touser+'】private msg：'+content+'</div></div>'
	);
	notifyMe();
})